function respconv=mlhTPABIsimfitEintParC(initparams,LUbounds,effb,resparamfix,onet3rkk,stateid,fitfu,nintm)
%
% resparam = mlhTPABIsimfitEintParC(initparams,LUbounds,effb,resparamfix,onet3rkk,stateid,fitfu,nintm)
%
% Optimize transition path parameters including the transition path time
% (t_TP) and the FRET efficiency of the transition path (E_TP) of an n-step, m-path TP model (n = 1, 2, or 3)
% by maximizing likelihood function calculated by mlhrateeigtrpathrevcalvitABIparC.m
%
...
% number of optimization parameters (= 3*nparI + 3):
...
% Eint (1:nparI): FRET efficiencies of the TPs, E_TP(1) - E_TP(nparI), each of which is the same for all TP steps (nintm)
...
% Output is a set of optimized parameters.
    ...
    function logmlh=mlhTPfitsub(params)
        ...
        nparI=(length(pconv)-3)/3;  % number of parallel pathways
        fretEF=pconv(1);
        fretEU=pconv(2);
        ktobrt=pconv(3);
